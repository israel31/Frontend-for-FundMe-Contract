...
const hexes$1 = Array.from({ length: 256 }, (v, i) =>
...
const sha3_224 = gen(6, 144, 224 / 8);
const sha3_256 = gen(6, 136, 256 / 8);
const sha3_384 = gen(6, 104, 384 / 8);
const sha3_512 = gen(6, 72, 512 / 8);
const keccak_224 = gen(1, 144, 224 / 8);
const keccak_256 = gen(1, 136, 256 / 8);
const keccak_384 = gen(1, 104, 384 / 8);
...
const shake128 = genShake(31, 168, 128 / 8);
const shake256 = genShake(31, 136, 256 / 8);
let locked$3 = false;
...
class Signature$1 {
  ...
  normalizeS() {
    return this.hasHighS()
      ? new Signature$1(this.r, mod(-this.s, CURVE.n))
      : this;
  }
  toDERRawBytes() {
...
class InternalSchnorrSignature {
  ...
  getScalar(priv) {
    ...
    return { point: point, scalar: scalar, x: point.toRawX() };
  }
  initNonce(d, t0h) {
...
function ens_normalize_fragment(frag, decompose) {
  ...
  return frag
    ...
    .map((label) =>
      str_from_cps(
        process(explode_cp(label), nf).flatMap((x) =>
          x.is_emoji ? filter_fe0f(x) : x
        )
      )
    )
    .join(STOP_CH);
...
function ens_tokenize(name, { nf = true } = {}) {
  ...
  while (input.length) {
    ...
    if (emoji) {
      ...
      let cp = input.pop();
      if (cp == STOP) {
        tokens.push({ type: TY_STOP, cp: cp });
      } else if (VALID.has(cp)) {
        tokens.push({ type: TY_VALID, cps: [cp] });
      } else if (IGNORED.has(cp)) {
        tokens.push({ type: TY_IGNORED, cp: cp });
      } else {
        let cps = MAPPED.get(cp);
        if (cps) {
          tokens.push({ type: TY_MAPPED, cp: cp, cps: cps.slice() });
        } else {
          tokens.push({ type: TY_DISALLOWED, cp: cp });
        }
      }
    }
  }
    ...
    for (let i = 0, start = -1; i < tokens.length; i++) {
      ...
      if (is_valid_or_mapped(token.type)) {
        if (requires_check(token.cps)) {
          ...
          let cps0 = slice.flatMap((x) =>
            is_valid_or_mapped(x.type) ? x.cps : []
          );
          let cps = nfc(cps0);
...
function buildWrappedFallback(contract) {
  const populateTransaction = async function (overrides) {
    ...
    assertArgument(
      payable || (tx.value || BN_0$1) === BN_0$1,
      "cannot send value to non-payable contract",
      "overrides.value",
      tx.value
    );
    assertArgument(
      iface.fallback || (tx.data || "0x") === "0x",
        ...
        (async () => {
          ...
          try {
            ...
            for (const { resolve, reject, payload } of batch) {
              ...
              if (resp == null) {
                return reject(
                  makeError("no response from server", "BAD_DATA", {
                    value: result,
...
class AES {
  constructor(key) {
    ...
    for (let i = 0; i < KC; i++) {
      ...
      __classPrivateFieldGet$4(this, _AES_Kd, "f")[rounds - index][i % 4] =
        tk[i];
    }
    let rconpointer = 0;
    let t = KC,
    ...
    for (let r = 1; r < rounds; r++) {
      for (let c = 0; c < 4; c++) {
        ...
        __classPrivateFieldGet$4(this, _AES_Kd, "f")[r][c] =
          U1[(tt >> 24) & 255] ^
          U2[(tt >> 16) & 255] ^
          U3[(tt >> 8) & 255] ^
          U4[tt & 255];
...
class CBC extends ModeOfOperation {
  ...
  encrypt(plaintext) {
    ...
    for (let i = 0; i < plaintext.length; i += 16) {
      for (let j = 0; j < 16; j++) {
        __classPrivateFieldGet$3(this, _CBC_lastBlock, "f")[j] ^=
          plaintext[i + j];
      }
      __classPrivateFieldSet$3(
        this,
        _CBC_lastBlock,
        this.aes.encrypt(__classPrivateFieldGet$3(this, _CBC_lastBlock, "f")),
        "f"
      );
  ...
  decrypt(ciphertext) {
    ...
    for (let i = 0; i < ciphertext.length; i += 16) {
      ...
      for (let j = 0; j < 16; j++) {
        plaintext[i + j] =
          block[j] ^ __classPrivateFieldGet$3(this, _CBC_lastBlock, "f")[j];
        __classPrivateFieldGet$3(this, _CBC_lastBlock, "f")[j] =
          ciphertext[i + j];
      }
    }
...
class CFB extends ModeOfOperation {
  ...
  encrypt(plaintext) {
    ...
    for (let i = 0; i < ciphertext.length; i += segmentSize) {
      const xorSegment = this.aes.encrypt(
        __classPrivateFieldGet$2(this, _CFB_shiftRegister, "f")
      );
      for (let j = 0; j < segmentSize; j++) {
        ciphertext[i + j] ^= xorSegment[j];
      }
      __classPrivateFieldGet$2(this, _CFB_instances, "m", _CFB_shift).call(
        this,
        ciphertext.subarray(i)
      );
    }
    return ciphertext;
  }
...
(_CFB_iv = new WeakMap()),
  (_CFB_shiftRegister = new WeakMap()),
  (_CFB_instances = new WeakSet()),
  (_CFB_shift = function _CFB_shift(data) {
    const segmentSize = this.segmentSize / 8;
    __classPrivateFieldGet$2(this, _CFB_shiftRegister, "f").set(
      __classPrivateFieldGet$2(this, _CFB_shiftRegister, "f").subarray(
        segmentSize
      )
    );
    __classPrivateFieldGet$2(this, _CFB_shiftRegister, "f").set(
      data.subarray(0, segmentSize),
      16 - segmentSize
    );
  });
...
class CTR extends ModeOfOperation {
  constructor(key, initialValue) {
    ...
    __classPrivateFieldSet$1(
      this,
      _CTR_remaining,
      __classPrivateFieldGet$1(this, _CTR_counter, "f"),
      "f"
    );
    ...
    for (let i = 0; i < crypttext.length; i++) {
      if (__classPrivateFieldGet$1(this, _CTR_remainingIndex, "f") === 16) {
        __classPrivateFieldSet$1(
          this,
          _CTR_remaining,
          this.aes.encrypt(__classPrivateFieldGet$1(this, _CTR_counter, "f")),
          "f"
        );
        __classPrivateFieldSet$1(this, _CTR_remainingIndex, 0, "f");
        this.increment();
      }
      crypttext[i] ^= __classPrivateFieldGet$1(this, _CTR_remaining, "f")[
        (__classPrivateFieldSet$1(
          this,
          _CTR_remainingIndex,
          ((_b = __classPrivateFieldGet$1(this, _CTR_remainingIndex, "f")),
          (_a = _b++),
          _b),
          "f"
        ),
    ...
    for (let i = 0; i < ciphertext.length; i++) {
      if (__classPrivateFieldGet(this, _OFB_lastPrecipherIndex, "f") === 16) {
        __classPrivateFieldSet(
          ...
          _OFB_lastPrecipher,
          this.aes.encrypt(
            __classPrivateFieldGet(this, _OFB_lastPrecipher, "f")
          ),
          "f"
        );
        __classPrivateFieldSet(this, _OFB_lastPrecipherIndex, 0, "f");
      }
      ciphertext[i] ^= __classPrivateFieldGet(this, _OFB_lastPrecipher, "f")[
        (__classPrivateFieldSet(
          this,
          _OFB_lastPrecipherIndex,
          ((_b = __classPrivateFieldGet(this, _OFB_lastPrecipherIndex, "f")),
          (_a = _b++),
          _b),
          "f"
        ),
...
(_OFB_iv = new WeakMap()),
  (_OFB_lastPrecipher = new WeakMap()),
  (_OFB_lastPrecipherIndex = new WeakMap());
